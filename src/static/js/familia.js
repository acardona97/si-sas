...
function validarDatosComunes(step) {
    if (step === 1) {
    ...
    if (step === 2) {
        const cards = document.querySelectorAll('#accionistas-container .accionista-card');
...
// ─── ACCIONISTAS ───
function agregarAccionistaComun() {
    accionistaCount++;
...
function removeAccionista(n) {
    ...
    if (card) card.remove();
    for (const clave of Object.keys(soportes)) {
        if (clave.endsWith(`:acc${n}`) || clave.endsWith(`:acc${n}_rl`)) delete soportes[clave];
    }
}
...
function addJuntaSuplente() {
    const disponibles = Array.from(document.querySelectorAll('#junta-principales-container [data-junta-row]'));
    const usados = new Set(Array.from(document.querySelectorAll('[data-suplente-de]')).map(el => el.value));
    const libre = disponibles.find((_,i) => !usados.has(String(i+1)));
    if (!libre) { alert('Cada miembro principal puede tener un suplente personal.'); return; }
    juntaSuplenteCount++;
    ...
    etiqueta.textContent = 'Suplente personal del miembro principal';
    const selector = document.createElement('select');
    selector.dataset.suplenteDe = '';
    disponibles.forEach((_,i) => selector.add(new Option(`Principal ${i+1}`, String(i+1))));
    selector.value = String(disponibles.indexOf(libre)+1);
...
function getAccionistasData() {
    const accionistas = [];
    document.querySelectorAll('#accionistas-container .accionista-card').forEach(card => {
...
async function generateDocuments() {
    for (let paso = 1; paso <= 6; paso++) { if (!validateStep(paso)) { showStep(paso); return; } }
...
// ─── SOCIEDAD DE FAMILIA ───
// Los campos comunes conservan el contrato de los anexos comerciales.
let clasesFamilia = [
    {codigo: 'A', denominacion: 'Acciones de fundadores', votos_por_accion: 10},
    {codigo: 'B', denominacion: 'Acciones ordinarias', votos_por_accion: 1},
];
let escenariosFamilia = {};
const escaparFamilia = valor => String(valor).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

function cambiarClases() {
    const cantidad = Number(document.getElementById('numero_clases').value);
    clasesFamilia = Array.from({length: cantidad}, (_, i) => clasesFamilia[i] || {
        codigo: String.fromCharCode(65+i), denominacion: 'Acciones ordinarias', votos_por_accion: 1,
    });
    renderClases();
}

function renderClases() {
    document.getElementById('clases-container').innerHTML = clasesFamilia.map((c,i) => `
        <fieldset><legend>Clase ${i+1}</legend><div class="form-row">
        <div class="form-group"><label for="clase-codigo-${i}">Código</label><input id="clase-codigo-${i}" value="${escaparFamilia(c.codigo)}" maxlength="8" onchange="editarClase(${i}, 'codigo', this.value)"></div>
        <div class="form-group"><label for="clase-denominacion-${i}">Denominación</label><input id="clase-denominacion-${i}" value="${escaparFamilia(c.denominacion)}" onchange="editarClase(${i}, 'denominacion', this.value)"></div>
        <div class="form-group"><label for="clase-votos-${i}">Votos por acción</label><input id="clase-votos-${i}" type="number" min="1" step="1" value="${c.votos_por_accion}" onchange="editarClase(${i}, 'votos_por_accion', this.value)"></div>
        </div></fieldset>`).join('');
    renderEscenarios();
    actualizarClasesAccionistas();
    actualizarOpcionesFamilia();
}

function editarClase(i, campo, valor) {
    const anterior = clasesFamilia[i].codigo;
    clasesFamilia[i][campo] = campo === 'votos_por_accion' ? Number(valor) : valor.trim();
    if (campo === 'codigo') {
        clasesFamilia[i].codigo = valor.trim().toUpperCase();
        document.getElementById(`clase-codigo-${i}`).value = clasesFamilia[i].codigo;
        // Se conserva la asignación del accionista al renombrar una clase.
        document.querySelectorAll('[data-clase-accionista]').forEach(s => {
            if (s.value === anterior) s.selectedOptions[0].value = clasesFamilia[i].codigo;
        });
        renderEscenarios();
        actualizarClasesAccionistas();
    }
...
function renderEscenarios() {
    const previos = escenariosFamilia;
    escenariosFamilia = {};
    const codigos = clasesFamilia.map(c => c.codigo);
    // El orden canónico coincide con itertools.combinations del backend.
    for (let cantidad = 1; cantidad <= codigos.length; cantidad++) {
        for (let mascara = 1; mascara < 2 ** codigos.length; mascara++) {
            const presentes = codigos.filter((_,i) => mascara & (1 << i));
            if (presentes.length !== cantidad) continue;
            const clave = presentes.join('+');
            const base = Math.round(10000 / cantidad) / 100;
            escenariosFamilia[clave] = Object.fromEntries(presentes.map((c,i) => [c,
                previos[clave]?.[c] ?? (i === cantidad-1 ? Math.round((100-base*(cantidad-1))*100)/100 : base)]));
        }
    }
    document.getElementById('escenarios-container').innerHTML = '<h3>Dividendos por escenarios</h3><p class="hint">Cada escenario debe sumar 100 %. Solo participan las clases indicadas.</p>' +
        Object.entries(escenariosFamilia).map(([key,reparto]) => `<fieldset data-escenario="${escaparFamilia(key)}"><legend>${escaparFamilia(key)}</legend><div class="form-row">${Object.entries(reparto).map(([c,pct]) => `<div class="form-group"><label>${escaparFamilia(c)} (%)<input type="number" min="0" max="100" step="0.01" data-dividendo="${escaparFamilia(c)}" value="${pct}"></label></div>`).join('')}</div><p data-total aria-live="polite"></p></fieldset>`).join('');
    document.querySelectorAll('[data-dividendo]').forEach(input => input.addEventListener('input', () => {
        escenariosFamilia[input.closest('[data-escenario]').dataset.escenario][input.dataset.dividendo] = input.value === '' ? NaN : Number(input.value);
        validarEscenarios();
    }));
    validarEscenarios();
}
...
function validarEscenarios() {
    let valido = true;
    document.querySelectorAll('[data-escenario]').forEach(row => {
        const valores = Object.values(escenariosFamilia[row.dataset.escenario]);
        const suma = valores.reduce((a,b) => a+b, 0);
        const ok = valores.every(v => Number.isFinite(v) && v >= 0 && v <= 100) && Math.abs(suma-100) < 0.01;
        row.querySelector('[data-total]').textContent = `${Number.isFinite(suma) ? suma.toFixed(2) : '—'} %${ok ? ' · Correcto' : ' · Debe sumar 100 %'}`;
        row.querySelector('[data-total]').className = ok ? 'upload-status success' : 'upload-status error';
        valido = valido && ok;
    });
    document.getElementById('escenarios-status').textContent = valido ? 'Todos los escenarios suman 100 %.' : 'Revise los porcentajes antes de continuar.';
    return valido;
}

function addAccionista() {
    agregarAccionistaComun();
    const n = accionistaCount;
    const card = document.getElementById(`acc_${n}`);
    card.insertAdjacentHTML('beforeend', `<div class="form-row"><div class="form-group"><label for="acc${n}_clase">Clase de acciones</label><select id="acc${n}_clase" name="acc${n}_clase" data-clase-accionista></select></div><div class="form-group"><label for="acc${n}_parentesco">Parentesco (obligatorio para persona natural)</label><input id="acc${n}_parentesco" name="acc${n}_parentesco" placeholder="Ej.: fundador, hija, cónyuge"></div></div>`);
    actualizarClasesAccionistas();
}

function actualizarClasesAccionistas() {
    document.querySelectorAll('[data-clase-accionista]').forEach(sel => {
        const previo = sel.value;
        sel.innerHTML = clasesFamilia.map(c => `<option value="${escaparFamilia(c.codigo)}">${escaparFamilia(c.codigo + ' — ' + c.denominacion)}</option>`).join('');
        if (clasesFamilia.some(c => c.codigo === previo)) sel.value = previo;
    });
}

function actualizarOpcionesFamilia() {
    document.getElementById('opcion-conversion_por_transferencia').classList.toggle('hidden', clasesFamilia.length === 1);
    document.getElementById('grupo-personalizado-block').classList.toggle('hidden', document.getElementById('grupo_familiar').value !== 'personalizado');
    document.getElementById('prohibicion-anios-block').classList.toggle('hidden', document.querySelector('[name="familia_prohibicion_temporal"]:checked').value !== 'true');
}

function collectAllData() {
    const d = recogerDatosComunes();
    d.modulo = 'familia';
    d.ciiu_code = '7010';
    d.ciiu_description = 'Actividades de administración empresarial';
    d.clases = clasesFamilia.map(c => ({...c, naturaleza: c.votos_por_accion > 1 ? 'acciones privilegiadas con voto múltiple' : 'acciones ordinarias'}));
    d.escenarios = JSON.parse(JSON.stringify(escenariosFamilia));
    d.elecciones = {};
    for (const key of ['grupo_familiar','grupo_personalizado','preferencia','prestamos','destino_renuncias']) d.elecciones[key] = document.getElementById(key).value.trim();
    for (const key of ['limite_rl_pct','prohibicion_anios']) d.elecciones[key] = Number(document.getElementById(key).value);
    d.opciones = {junta_directiva: d.tiene_junta};
    document.querySelectorAll('input[name^="familia_"]:checked').forEach(r => { d.opciones[r.name.replace('familia_','')] = r.value === 'true'; });
    if (clasesFamilia.length === 1) d.opciones.conversion_por_transferencia = false;
    for (const key of ['capital_autorizado','capital_suscrito','valor_nominal']) d[key] = _parseCop(d[key]);
    d.accionistas.forEach(a => {
        a.clase = document.getElementById(`${a.doc_key}_clase`).value;
        a.parentesco = document.getElementById(`${a.doc_key}_parentesco`).value.trim();
        a.capital_pagado_num = a.capital_pagado.trim() === '' ? Math.round(d.capital_suscrito * a.porcentaje / 100) : _parseCop(a.capital_pagado);
        // Se envían todos los aportes explícitos: el backend trata los vacíos
        // como cero cuando otro accionista sí indicó su capital pagado.
        a.capital_pagado = String(a.capital_pagado_num);
    });
    d.capital_pagado = d.accionistas.reduce((s,a) => s+a.capital_pagado_num, 0);
    if (d.junta_directiva) {
        const asociaciones = new Map();
        document.querySelectorAll('#junta-suplentes-container [data-junta-row]').forEach(fila => {
            const key = fila.querySelector('[name$="_nombre"]').name.replace(/_nombre$/, '');
            asociaciones.set(key, Number(fila.querySelector('[data-suplente-de]').value));
        });
        d.junta_directiva.suplentes.forEach(p => { p.principal_doc_key = `jd_pri_${asociaciones.get(p.doc_key)}`; });
        d.junta_directiva.suplentes.sort((a,b) => asociaciones.get(a.doc_key)-asociaciones.get(b.doc_key));
    }
    d.es_empresa_familiar = false;
    d.nucleo_familiar = [];
    return d;
}

function validateStep(paso) {
    const error = texto => { alert(texto); return false; };
    if (paso === 1) {
        for (const id of ['municipio','departamento','camara_ciudad','direccion','barrio','email','telefono1']) {
            const campo = document.getElementById(id);
            if (!campo.value.trim() || !campo.checkValidity()) return error(`Complete correctamente el campo ${document.querySelector(`label[for="${id}"]`)?.textContent || id}.`);
        }
    }
    if (paso === 2) {
        const codigos = clasesFamilia.map(c => c.codigo);
        if (new Set(codigos).size !== codigos.length || clasesFamilia.some(c => !/^[A-Z][A-Z0-9_]*$/.test(c.codigo) || !c.denominacion || !Number.isInteger(c.votos_por_accion) || c.votos_por_accion < 1)) return error('Revise las clases: códigos únicos, denominación y votos enteros positivos.');
        if (!validarEscenarios()) return error('Cada escenario de dividendos debe sumar 100 %.');
        for (const a of getAccionistasData()) {
            if (a.tipo === 'natural' && !document.getElementById(`${a.doc_key}_parentesco`).value.trim()) return error(`Indique el parentesco de ${a.nombre}.`);
            if (a.tipo === 'juridica' && (!a.rl_nombre || !a.rl_cc)) return error('Complete el representante legal de cada accionista persona jurídica.');
        }
    }
    if (paso === 3) {
        const representantes = [...getRepresentantes('principal'), ...getRepresentantes('suplente')];
        if (representantes.some(p => !p.cc)) return error('Complete el documento de cada representante legal designado.');
    }
    if (paso === 5) {
        const d = collectAllData();
        for (const a of d.accionistas) {
            const capital = d.capital_suscrito * a.porcentaje / 100;
            if (Math.abs(capital/d.valor_nominal - Math.round(capital/d.valor_nominal)) > 0.000001) return error('Ajuste los porcentajes: cada accionista debe suscribir acciones enteras.');
            if (a.capital_pagado_num < 0 || a.capital_pagado_num > capital) return error('El capital pagado debe estar entre cero y el suscrito por cada accionista.');
        }
    }
    if (paso === 6) {
        const d = collectAllData();
        if (d.tiene_junta) {
            const asociaciones = Array.from(document.querySelectorAll('[data-suplente-de]')).map(s=>Number(s.value));
            const n = Number(document.getElementById('junta_num_principales').value);
            if (new Set(asociaciones).size !== asociaciones.length || asociaciones.some(i => i < 1 || i > n)) return error('Asigne cada suplente a un principal distinto y existente.');
            if (getJuntaPersonas('junta-suplentes-container',true).some(p=>!p.nombre || !p.id_num)) return error('Complete los suplentes de junta o elimine las filas vacías.');
        }
        if (d.tiene_revisor && document.getElementById('revisor_tiene_suplente').checked && !getRevisorSuplente()) return error('Complete el documento y la tarjeta profesional del revisor suplente.');
        if (d.elecciones.grupo_familiar === 'personalizado' && !d.elecciones.grupo_personalizado) return error('Escriba la definición personalizada del grupo familiar.');
        const limite = document.getElementById('limite_rl_pct');
        if (!limite.value || !limite.checkValidity()) return error('El límite del representante debe ser un porcentaje entero entre 0 y 100.');
        const anios = d.elecciones.prohibicion_anios;
        if (d.opciones.prohibicion_temporal && (!Number.isInteger(anios) || anios < 1 || anios > 10)) return error('La prohibición debe durar entre 1 y 10 años.');
        if (document.querySelector('[name="tiene_apoderado"]:checked').value === 'si' && (!d.apoderado?.nombre || !d.apoderado?.id_num)) return error('Complete el nombre y documento del apoderado.');
    }
    return validarDatosComunes(paso);
}

function buildSummary() {
    // Se usa textContent para que ningún dato digitado se interprete como HTML.
    const d = collectAllData();
    const contenedor = document.getElementById('summary-content');
    contenedor.replaceChildren();
    const agregar = (titulo, valor) => {
        const fila = document.createElement('div');
        fila.className = 'summary-field';
    ...
    agregar('Sociedad de familia', d.nombre_sas);
    agregar('Domicilio', `${d.direccion}, ${d.barrio}, ${d.municipio}, ${d.departamento}. Cámara: ${d.camara_ciudad}`);
    agregar('Contacto', [d.email,d.telefono1,d.telefono2,d.telefono3].filter(Boolean).join(' · '));
    agregar('CIIU principal', '7010 — Actividades de administración empresarial');
    agregar('CIIU secundario', `${d.ciiu_code_sec} ${d.ciiu_description_sec}`);
    agregar('Objeto social', d.objeto_social);
    agregar('Capital', `Autorizado: ${d.capital_autorizado}; suscrito: ${d.capital_suscrito}; nominal: ${d.valor_nominal}`);
    agregar('Régimen', `${d.regimen}; ingresos mensuales: ${d.ingresos_mensuales}`);
    d.clases.forEach(c => agregar(`Clase ${c.codigo}`, `${c.denominacion}, ${c.votos_por_accion} votos por acción`));
    Object.entries(d.escenarios).forEach(([k,v]) => agregar(`Dividendos ${k}`, Object.entries(v).map(([c,p])=>`${c}: ${p} %`).join(' · ')));
    d.accionistas.forEach(a => agregar(a.nombre, `${a.id_num}, ${a.porcentaje} %, clase ${a.clase}, pagado: ${a.capital_pagado_num}, parentesco: ${a.parentesco || 'No aplica'}`));
    for (const [titulo,personas] of [['Representante principal',d.rl_principales],['Representante suplente',d.rl_suplentes],['Junta principal',d.junta_directiva?.principales || []],['Junta suplente personal',d.junta_directiva?.suplentes || []]]) personas.forEach(p=>agregar(titulo,`${p.nombre}, ${p.cc || p.id_num}`));
    for (const [titulo,p] of [['Revisor fiscal',d.revisor_fiscal],['Apoderado',d.apoderado]]) if (p) agregar(titulo, JSON.stringify(p));
    Object.entries(d.opciones).forEach(([k,v])=>agregar(k.replaceAll('_',' '),v ? 'Sí' : 'No'));
    Object.entries(d.elecciones).forEach(([k,v])=>agregar(k.replaceAll('_',' '),v));
    agregar('Pendiente de revisión jurídica', 'No se pactan opción de compra ni exclusión.');
}

document.addEventListener('DOMContentLoaded', () => {
    renderClases();
    document.getElementById('grupo_familiar').addEventListener('change', actualizarOpcionesFamilia);
    document.querySelectorAll('input[name^="familia_"]').forEach(r => r.addEventListener('change', () => {
        document.querySelectorAll(`input[name="${r.name}"]`).forEach(el=>el.closest('.radio-card').classList.toggle('selected', el.checked));
        actualizarOpcionesFamilia();
    }));
});
